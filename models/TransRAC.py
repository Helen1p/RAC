...
class shift(nn.Module):
    # shape of x: [32, 64, 512]
    def __init__(self, fold_div=8, pad_zero=True, group_size=16):
        super().__init__()
    ...
    def forward(self, x):
        ...
        c = x.shape[-1]
        t = x.shape[-2]
        for i in range(0, t, self.group_size):
            x_1 = x[:, i:i + self.group_size, :].clone()
            out = torch.zeros_like(x_1)
            fold = c // self.fold_div
            out[:, :-1, :fold] = x_1[:, 1:, :fold]  # 向上shift
            out[:, 1:, fold: 2 * fold] = x_1[:, :-1, fold: 2 * fold]  # 向下shift
            out[:, :, 2 * fold:] = x_1[:, :, 2 * fold:]  # not shift
            if self.pad:
                out[:, -1, :fold] = 0
                out[:, 0, fold: 2 * fold] = 0
            else:
                out[:, -1, :fold] = x_1[:, 0, :fold]
                out[:, 0, fold: 2 * fold] = x_1[:, -1, fold: 2 * fold]
            list_.append(out)
        out = torch.cat(list_, dim=-2) # [32, 64, 125]

        conv_out = self.conv(out.permute(0,2,1)).permute(0,2,1)
        out = conv_out + x
        out = F.relu(out)
        # out = torch.unsqueeze(out,1) # [32,1,64,125]
        # out = self.conv(out)
        # out = torch.squeeze(out,1) # [32,64,125]
        # print(out.shape)
        # out += x
        # out = F.relu(out)
        return out
