    ...
    for epoch in tqdm(range(currEpoch, n_epochs + currEpoch)):
        ...
        if inference:
            with torch.no_grad():
                ...
                for input, target, video_file_name in pbar:
                    model.eval()
                    acc = 0
                    input = input.to(device)
                    count = torch.sum(target, dim=1).round().to(device)
                    output, sim_matrix, output_o, matrix1, matrix2 = model(input)
                    predict_count = torch.sum(output, dim=1).round()
                    v_name = video_file_name[0]
                    # np.save('./results/{}.npy'.format(v_name), output_o.detach().cpu().numpy())
                    # np.save('./results/{}.npy'.format(v_name), output.detach().cpu().numpy())
                    np.save('./results/{}.npy'.format(v_name), matrix2.detach().cpu().numpy())
